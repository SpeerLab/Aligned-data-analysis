...
outpath = base_folder;
%
load([base_folder 'R_add_to_statsVCw10_edges_in.mat'],'tintsG_p140');
...
%
load([outpath 'statsR2w10_edges_plus_VC_in.mat']);
statsRwater_sssss = statsGwater(pairedg_idx);
statsRwater_ssssn = statsGwater(pairedg_idx2);
save([base_folder 'R_paired_VC_in.mat'],'statsRwater_sssss','statsRwater_ssssn');
%%
save_path = 'X:\Chenghang\4_Color\Raw\1.12.2021_P8EC_A\analysis\Result\Data\';
temp = [statsRwater_sssss.Area];
temp = temp';
save([save_path 'statsRwater_sssss.Area_in.txt'],'temp','-ascii','-double');
temp = [statsRwater_sssss.TintsG];
temp = temp';
save([save_path 'statsRwater_sssss.TintsG_in.txt'],'temp','-ascii','-double');
temp = [statsRwater_ssssn.Area];
temp = temp';
save([save_path 'statsRwater_ssssn.Area_in.txt'],'temp','-ascii','-double');
temp = [statsRwater_ssssn.TintsG];
temp = temp';
save([save_path 'statsRwater_ssssn.TintsG_in.txt'],'temp','-ascii','-double');
