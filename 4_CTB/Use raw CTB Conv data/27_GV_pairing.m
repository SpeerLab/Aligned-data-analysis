...
outpath = base_folder;
%
load([base_folder 'G_add_to_statsVCw10_edges_out.mat'],'tintsG_p140');
...
%
load([outpath 'statsG2w10_edges_plus_VC_out.mat']);
statsGwater_sssss = statsGwater(pairedg_idx);
statsGwater_ssssn = statsGwater(pairedg_idx2);
save([base_folder 'G_paired_VC_out.mat'],'statsGwater_sssss','statsGwater_ssssn');
%%
save_path = 'X:\Chenghang\4_Color\Raw\1.12.2021_P8EC_A\analysis\Result\Data\';
temp = [statsGwater_sssss.Area];
temp = temp';
save([save_path 'statsGwater_sssss.Area_out.txt'],'temp','-ascii','-double');
temp = [statsGwater_sssss.TintsG];
temp = temp';
save([save_path 'statsGwater_sssss.TintsG_out.txt'],'temp','-ascii','-double');
temp = [statsGwater_ssssn.Area];
temp = temp';
save([save_path 'statsGwater_ssssn.Area_out.txt'],'temp','-ascii','-double');
temp = [statsGwater_ssssn.TintsG];
temp = temp';
save([save_path 'statsGwater_ssssn.TintsG_out.txt'],'temp','-ascii','-double');