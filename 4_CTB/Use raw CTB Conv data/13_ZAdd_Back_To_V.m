...
clear all;clc
%
base_path = 'Z:\Chenghang\12.21.2020_P8EA_A\';
...
%
load([outpath 'V_paired_C_in.mat']);
...
ad_thresh = adaptthresh(BG(:,:,i),sen);
DYs(:,:,i) = imbinarize(BG(:,:,i),ad_thresh);
end
%BP2 = BP;
parfor i = 1:num_images
    disp(int2str(i))
    BP2(:,:,i) = uint8(DYs(:,:,i)).*BG(:,:,i);
end
%%
disp('loading data')
%
...
%
save([outpath 'V_paired_C_in.mat'],'statsVwater_ss','statsVwater_sn');
%%
save_path = 'Z:\Chenghang\12.21.2020_P8EA_A\analysis\Result\Data\';
temp = [statsVwater_ss.Area];
temp = temp';
save([save_path 'statsVwater_sss.Area_in.txt'],'temp','-ascii','-double');
temp = [statsVwater_ss.TintsG];
temp = temp';
save([save_path 'statsVwater_sss.TintsG_in.txt'],'temp','-ascii','-double');
temp = [statsVwater_sn.Area];
temp = temp';
save([save_path 'statsVwater_ssn.Area_in.txt'],'temp','-ascii','-double');
temp = [statsVwater_sn.TintsG];
temp = temp';
save([save_path 'statsVwater_ssn.TintsG_in.txt'],'temp','-ascii','-double');