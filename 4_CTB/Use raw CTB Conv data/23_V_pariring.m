...
%
load([base_folder 'statsV2w10_edges_plus_out.mat']);
voxel = [15.5 15.5 70];
...
%%
load([base_folder 'add_to_statsVw10_edges_out.mat'],'tintsG_p140');
...
Eps = 0.1726; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
savefig([base_folder 'storm_gs_ps_mints_nn_shell_2d_hist_out.fig'])
...
%
load([outpath 'statsV2w10_edges_plus_out.mat']);
statsVwater_ss = statsGwater(pairedg_idx);
statsVwater_sn = statsGwater(~pairedg_idx);
save([base_folder 'V_paired_C_out.mat'],'statsVwater_ss','statsVwater_sn');
