clear;clc
%
base_folder = 'X:\Chenghang\4_Color\Raw\1.11.2021_B2P8C_B\analysis\Result\4_CTB\';
outpath = base_folder;
...
%
load([base_folder 'statsV2w10_edges_plus_in.mat']);
voxel = [15.5 15.5 70];
...
%%
load([base_folder 'add_to_statsVw10_edges_in.mat'],'tintsG_p140');
...
Eps = 0.0859; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
%
load([outpath 'statsV2w10_edges_plus_in.mat']);
statsVwater_ss = statsGwater(pairedg_idx);
statsVwater_sn = statsGwater(~pairedg_idx);
save([base_folder 'V_paired_C_in.mat'],'statsVwater_ss','statsVwater_sn');