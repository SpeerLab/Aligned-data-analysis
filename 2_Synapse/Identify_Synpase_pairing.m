clear;clc
exp_folder = 'Y:\Chenghang\04_4_Color\Control_Group\7.1.20.WT_P2_CB_B\';
...
Eps = 0.2445; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
save([base_folder 'G_paired.mat'],'statsGwater_ss','statsGwater_sn','-v7.3');
%
...
% figure; plot(CDg(orderg))
%% get dbscan clusters from optics threshold and plot (geph)
Eps = 0.2404; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
            clustID = clustID + 1; classg(orderg(i)) = clustID;
