...
clear all;clc
%
data_path = 'X:\Chenghang\Backup_Raw_Data\Shell_Analysis\data_p70_CTBneg_Normalized\';
...
%    statsVwater(jj).statxtureP_70pos = statxture([RPixelValues_m140(posvals2)]);
   ...
   area_BP1 = numel(find(curr2));
   area_shell = statsVwater(jj).area_p140 - area_BP1;
   Normalized_area(jj) = statsVwater(jj).volume_p140 / area_shell;
   Normalized_tints(jj) = statsVwater(jj).tints_p140 / area_shell;
...
save([data_path 'Area_CTBneg_Norm_' sprintf('%03d',cur_path) '.txt'],'temp','-ascii','-double');

temp = tints_p140_norm;

save([data_path 'Tints_CTBneg_Norm_' sprintf('%03d',cur_path) '.txt'],'temp','-ascii','-double');
