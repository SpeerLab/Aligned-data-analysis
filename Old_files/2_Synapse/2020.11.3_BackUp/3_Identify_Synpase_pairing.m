...
%
expfolder = [exp_folder '\analysis\elastic_align\storm_merged\'];
...
Eps = 0.4295; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
statsGwater_ss = statsGa2s(pairedg_idx);
statsGwater_sn = statsGa2s(~pairedg_idx);
save([base_folder 'G_paired.mat'],'statsGwater_ss','statsGwater_sn');
%%
...
% figure; plot(CDg(orderg))
%% get dbscan clusters from optics threshold and plot (geph)
Eps = 1.05; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
            clustID = clustID + 1; classg(orderg(i)) = clustID;
